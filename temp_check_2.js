  ...
  const defaultBatchDate = 'jinja_var';
  ...
  const expensePercent = Number('jinja_var') || 0;
  const hideSalePrices = 'jinja_var';
  ...
  const setConfirmData = ()=>{
    ...
    // Pending amount UI logic
    const hasCustomer = customerText !== "Walk-in customer" && customerText !== "Select customer";
    const confirmAmountPaidRow = document.getElementById("confirmAmountPaidRow");
    const confirmAmountPaidInput = document.getElementById("confirmAmountPaidInput");
    const confirmPendingAmount = document.getElementById("confirmPendingAmount");
    
    if(confirmAmountPaidRow && confirmAmountPaidInput && confirmPendingAmount){
      if(hasCustomer){
        confirmAmountPaidRow.style.display = "flex";
        confirmAmountPaidInput.value = data.totalAmount;
        confirmAmountPaidInput.max = data.totalAmount;
        confirmPendingAmount.textContent = "PKR 0.00";
        
        confirmAmountPaidInput.oninput = (e) => {
           let paid = Number(e.target.value);
           if(paid > data.totalAmount) { paid = data.totalAmount; e.target.value = paid; }
           if(paid < 0 || !Number.isFinite(paid)) { paid = 0; }
           const pending = data.totalAmount - paid;
           confirmPendingAmount.textContent = formatMoney(pending);
        };
      } else {
        confirmAmountPaidRow.style.display = "none";
      }
    }
  };
  ...
  const submitSaleForm = async ()=>{
    ...
    const confirmAmountPaidRow = document.getElementById("confirmAmountPaidRow");
    const confirmAmountPaidInput = document.getElementById("confirmAmountPaidInput");
    const multiSalePendingAmount = document.getElementById("multiSalePendingAmount");
    const data = getConfirmData();
    
    if (multiSalePendingAmount) {
       if (confirmAmountPaidRow && confirmAmountPaidRow.style.display !== "none" && confirmAmountPaidInput) {
          const paid = Number(confirmAmountPaidInput.value) || 0;
          multiSalePendingAmount.value = Math.max(0, data.totalAmount - paid);
       } else {
          multiSalePendingAmount.value = 0;
       }
    }
  ...
  const customerLookup = 'jinja_var';
  ...
  const profitModalOpen = 'jinja_var';
  const financeModalOpen = 'jinja_var';
  const financeCategoryOpen = 'jinja_var';
  const financeNetworkOpen = 'jinja_var';
  const managerActivePage = 'jinja_var';
  const managerCatalogTab = 'jinja_var';
  ...
  activateWalletHistoryTab("'jinja_var'");
  ...
  window.openCustomerHistoryModal = function(id, name, balance) {
    ...
    tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px;">Loading history...</td></tr>';
      ...
      .then(data => {
        if (data.status === 'success') {
          if (data.history.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px; color: var(--muted);">No history available.</td></tr>';
            return;
          }
          let html = '';
          data.history.forEach((row, index) => {
            const idx = data.history.length - index;
            ...
            if (row.sale_items && row.sale_items.length > 0) {
              ...
              // Store items string safely in dataset for robust modal rendering
              const safeItemsJSON = JSON.stringify(row.sale_items).replace(/"/g, '&quot;');
              itemsHtml = `<button type="button" class="btn btn-soft btn-mini" style="font-size: 11px; padding: 4px 8px; font-weight: bold;" onclick="showCustomerHistoryItems(this)" data-items="${safeItemsJSON}">${totalItems} items ℹ️</button>`;
            }
  ...
  // Enforce +92 prefix for phone inputs
  document.querySelectorAll('input[name="phone"], input[name="customer_phone"]').forEach(input => {
    input.addEventListener('input', function(e) {
      if (this.value.indexOf('+92') !== 0) {
        // If they messed up the prefix, reset it and try to salvage the digits they typed
        ...
        this.value = '+92' + digits;
      }
    });
    
    input.addEventListener('keydown', function(e) {
      // Prevent backspacing into the +92 prefix
      if (e.key === 'Backspace' && this.selectionStart <= 3 && this.selectionEnd === this.selectionStart) {
        e.preventDefault();
      }
      // Prevent moving cursor before +92
      if (e.key === 'ArrowLeft' && this.selectionStart <= 3 && this.selectionEnd === this.selectionStart) {
        e.preventDefault();
      }
    });
    
    input.addEventListener('click', function(e) {
      // If user clicks before +92, move cursor to the end of +92
